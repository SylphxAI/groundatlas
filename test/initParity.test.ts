...
beforeEach(async () => {
  tempRoot = await mkdtemp(path.join(os.tmpdir(), "groundatlas-init-parity-"));
...
function runCliInit(
  ...
  return spawnSync("bun", ["run", path.join(repoRoot, "src/cli.ts"), "init"], {
    cwd: fixtureRoot,
    encoding: "utf8",
    env,
    stdio: ["ignore", "pipe", "pipe"],
...
test("CLI init defaults to Rust authority when GROUNDATLAS_RUST_SCANNER is unset", () => {
  const result = runCliInit({}, { omitRustScannerFlag: true });

  expect(result.status).toBe(0);
  expect(result.stdout).toContain("GroundAtlas initialized .groundatlas/");
...
test("CLI init delegates full AtlasMap when GROUNDATLAS_RUST_SCANNER=1", () => {
  const result = runCliInit({ GROUNDATLAS_RUST_SCANNER: "1" });
...
test("CLI init can opt out to TS when GROUNDATLAS_RUST_SCANNER=ts", () => {
  const result = runCliInit({ GROUNDATLAS_RUST_SCANNER: "ts" });
